...
#define RAND() ((rand()%1001)/1000.0f)

/*
Constructor, which sets everything to some 'sensible' defaults.
*/
ParticleEmitter::ParticleEmitter(GLuint texture, float rad)	{
	particleRate		= 10.0f;
	...
	largestSize			= 0;
	emitterRadius		= rad;
	emitterPosition		= Vector3(0,0,0);
	destructionHeight	= 0.0f;
...
void ParticleEmitter::Update(float msec)	{
	...
	*/
	...
	for(std::vector<Particle *>::iterator i = particles.begin(); i != particles.end();/*No I++ here!!! */) {
		...
		//if(p->colour.w <= 0.0f) {
		if (p->position.y <= destructionHeight) {
