...
void Mesh::SetColour(Vector4 colour)
{
	...
	glMapBuffer(GL_ARRAY_BUFFER, bufferObject[COLOUR_BUFFER]);
...
void Mesh::BufferData()
{
	...
	if (textureCoords) {
		...
		glGenBuffers(1, &bufferObject[TEXTURE2_BUFFER]);
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[TEXTURE2_BUFFER]);
		glBufferData(GL_ARRAY_BUFFER, numVertices * sizeof(Vector2), textureCoords, GL_STATIC_DRAW);
		glVertexAttribPointer(TEXTURE2_BUFFER, 2, GL_FLOAT, GL_FALSE, 0, 0);
		glEnableVertexAttribArray(TEXTURE2_BUFFER);
	}
	if (colours) {
